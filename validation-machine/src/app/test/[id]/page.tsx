...
export default function TestDetailPage() {
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      await fetch(`/api/tests/${params.id}/generate-images`, { method: 'POST' })
      await fetchTest()
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* Step 1: Generate Concept Images (pending) */}
        {test.status === 'pending' && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 animate-fadeIn">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-violet-400 text-white flex items-center justify-center font-bold text-lg shadow-md">
                1
              </div>
              <div className="flex-1">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-violet-500 text-white rounded-xl font-medium hover:-translate-y-0.5 hover:shadow-lg shadow-md disabled:opacity-50 disabled:hover:translate-y-0 flex items-center gap-2"
              >
                {generatingImages ? (
                  <>
                    ...
                    {variant.imageUrl && (
                      <div className="mb-4 rounded-lg overflow-hidden border border-slate-100">
                        ...
                        <img
                          ...
                          alt={`Ad creative for variant ${variant.name}`}
                          className="w-full aspect-[1200/628] object-cover"
                        />
                      </div>
                    )}

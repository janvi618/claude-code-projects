...
|--------------------|----------------------------|-------------|
| `llm_model`        | Any model identifier string | Provider-native model ID (e.g., `gpt-5.4`, `claude-opus-4-6`) |
...
1. Explicit node attribute (e.g., `llm_model="gpt-5.4"` on the node) -- highest precedence
    ...
    graph [
        ...
        model_stylesheet="
            * { llm_model: claude-sonnet-4-5; llm_provider: anthropic; }
            ...
            #critical_review { llm_model: gpt-5.4; llm_provider: openai; reasoning_effort: high; }
        "
    ]
...
- `critical_review` gets `gpt-5.4` and `reasoning_effort: high` from the `#critical_review` rule (specificity 3, overriding both `.code` and `box`).
